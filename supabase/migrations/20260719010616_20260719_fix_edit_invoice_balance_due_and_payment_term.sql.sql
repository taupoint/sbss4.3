/* Fix edit_invoice: balance_due is a generated column (cannot UPDATE), and add payment term + method support with full accounting reversal like cancel_invoice. */
...
BEGIN
  ...
  -- Determine old payment term from invoice status
  ...
  -- STEP 1: Restore stock (subtract already-returned qty to avoid double-credit)
  ...
  -- STEP 4: Reverse original payments — create reversal payment records AND delete original payment JEs with account balance rollback
  ...
  -- STEP 9: Deduct stock for new items
  FOR v_i IN SELECT generate_series(0, json_array_length(v_new_items) - 1) LOOP
    v_new_item := v_new_items->v_i;
    ...
    IF v_default_wh IS NOT NULL AND v_qty > 0 THEN
      ...
      INSERT INTO stock_movements (product_id, warehouse_id, movement_type, quantity, unit_cost, reference_type, reference_id, reference_number, notes)
      VALUES ((v_new_item->>'product_id')::uuid, v_default_wh, 'sale_out', v_qty, COALESCE((v_new_item->>'cost_price')::numeric, 0), 'invoice_edit', p_invoice_id, v_invoice.invoice_number, 'Stock deduction - invoice edited');
    END IF;
  END LOOP;

  -- STEP 10: Apply new payment term
  IF v_new_payment_term = 'credit' THEN
    -- Credit: no payment, status = sent
  ...
  ELSIF v_new_payment_term = 'partial' THEN
    ...
    IF v_new_partial_amount > 0 THEN
      ...
      -- Post payment JE (debit cash, credit AR)
      IF v_cash_account IS NOT NULL AND v_ar_account IS NOT NULL THEN
        PERFORM post_journal_entry(
          'Payment - Invoice ' || v_invoice.invoice_number || ' EDITED', CURRENT_DATE, 'payment',
          (SELECT id FROM payments WHERE reference_type = 'invoice' AND reference_id = p_invoice_id ORDER BY created_at DESC LIMIT 1),
          json_build_array(
    ...
    END IF;
  ELSE
    -- Full payment: record full payment
  ...
  -- STEP 12: Update customer outstanding_balance and total_purchases
